...
Page({
  ...
  bindMultiPickerChange (e) {
    ...
    this.data.addressList.forEach((i, oi) => {
      if (oi === value[0]) {
        locationId.push(i.areaId)
        i.cities.forEach((s, os) => {
          if (os === value[1]) {
            locationId.push(s.areaId)
            s.counties.forEach((t, ot) => {
              if (ot === value[2]) {
                locationId.push(t.areaId)
              }
            })
          }
        })
    ...
    this.setData({
      multiIndex: value,
      'addressData.locationId': locationId.join(',')
    })
  },
  toSetAuthor (e) {
    console.log(e)
  ...
  addressTxtInput (e) {
    this.setData({
      'addressData.addressTxt': e.detail.value
    })
  },
  switchChange (e) {
    console.log(e)
  ...
  toAddAddress () {
    if (this.data.addressData.name === '') {
      app.alert.error('请输入收货人姓名');
      return
    }

    if (this.data.addressData.phone === '') {
      app.alert.error('请输入收货人号码')
      return
    }

    if (this.data.addressData.locationId === '') {
      app.alert.error('请选择所在地区')
      return
    }

    if (this.data.addressData.addressTxt === '') {
      app.alert.error('请填写详细地址')
      return
    }
